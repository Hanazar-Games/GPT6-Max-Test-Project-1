...
export function mountExpeditionUI() {
  $('mode-options').insertAdjacentHTML('beforeend', '<button data-mode="expedition" aria-pressed="false"><span>远征补给<small>沿途委托 · 改装成长 · 三站远征</small></span><b>◎</b></button>');
  $('cup-brief').insertAdjacentHTML('afterend', '<div id="expedition-brief" class="expedition-brief" hidden><strong>把沿途的收获，变成下一程的底气。</strong><p>依次穿越宁静海、永夜峡谷与曙光前线。每站交付获 2 补给点；额外核心、精准过门与勘探委托每项再获 2 点。委托可自由取舍，交付成功才结算。</p><div><span>01 / 完成委托</span><span>02 / 补给改装</span><span>03 / 深入月背</span></div><small>两次中途补给，可升级引擎、充能、装甲或吸附范围，每项最多两级。失败可重试；返回基地或刷新结束远征。远征不计入个人幽灵纪录。</small></div>');
...
export function renderExpeditionHUD(run, game) {
  $('expedition-stage').textContent = `远征 / ${run.stage + 1} OF 3`;
...
export function renderExpeditionResult(run, game) {
  ...
  if (won) {
    ...
    $('result-title').textContent = complete ? '月背走过，满载归来。' : '补给已到，整装再出发。';
    $('result-copy').textContent = complete ? `${game.craft.name}已完成 6.2 公里远征，三座基地的能量全部送达。` : `${game.mission.name}交付完成。下一站 ${MISSIONS[run.stage + 1].name}；启航时补满装甲与能量。`;
  ...
  if (complete) {
    ...
    $('expedition-summary').innerHTML = `<strong>${contracts} / 9 委托完成 · ${earned} 补给赚取</strong><p>成功航程 ${time.toFixed(2)}s · 改装花费 ${earned - run.supply} 点 · 结余 ${run.supply} 点</p>`;
  }
