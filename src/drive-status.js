const STATUS = {
  ...
  overdrive: { kind: 'overdrive', text: '超频道具 · 免费冲刺中，S 可制动' },
...
export function getDriveStatus(game, { brake = false, cruise = false, lowGravity = false } = {}) {
  const kind = brake ? 'brake' : game.overdriveTime > 0 ? 'overdrive' : game.padBoost > 0 ? 'pad' : game.boosting ? 'boost' : game.boostLocked ? 'locked' : cruise ? 'cruise' : lowGravity ? 'gravity' : game.speed < 3 ? 'idle' : 'moving';
  return STATUS[kind];
}
